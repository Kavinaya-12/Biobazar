...
const PersonalCare = () => {
  ...
  const { personalCare, filteredProducts } = useSelector((state) => state.products);
  const wishlist = useSelector((state) => state.wishlist.items);

  const [addedToCart, setAddedToCart] = useState({});

  const section = "PersonalCare";
  ...
  const productsToShow = filteredProducts?.length ? filteredProducts : personalCare;

  return (
   <div className="personalcare-container">
  <h1 className="personalcare-title">Personal Care Products</h1>
  <SearchBar />
  <div className="personalcare-grid">
    {productsToShow.map((product) => {
      ...
      const isInWishlist = wishlist.some((i) => i._key === _key);
      return (
        <div key={_key} className="personalcare-card">
